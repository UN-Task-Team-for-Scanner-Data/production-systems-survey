...
[^concepts-2]: For readers unfamiliar with the CPI domain, Elementary Aggregates (or EAs) are the lowest level of groups of goods and services for which expenditure shares are available and serve as the building blocks of the CPI. Calculated EAs are aggregated using higher-level indices using expenditure shares as weights to make up the CPI. Most NSOs use a Laspeyres type higher-level price index, however elementary aggregates can be very nuanced and require their own approaches depending on the data and method that needs to be used. See [sections 8.7 - 8.90 in Chapter 8 of the CPI Manual](https://www.imf.org/-/media/Files/Data/CPI/cpi-manual-concepts-and-methods.ashx) for more info.

[^concepts-3]: This step refers to the process of aggregating Elementary Aggregataes to obtain higher-level price indices, all the way to the all items CPI. Aggregation typically uses the CIOCOP classification structure. See [sections 8.91 - 8.96 of Chapter 8, in the CPI Manual](https://www.imf.org/-/media/Files/Data/CPI/cpi-manual-concepts-and-methods.ashx) for more info.
