...
The whole point of this section is to concretely specify a joint team organization **and** system topology.

Since these joint team/system topology labels can be attached to individual records **and** we have business outcomes associated with these same records, we can ask meaningful questions about how team and system organization are related to business outcomes.

**Caveat**: due to the small sample size and limited nature of questions we were able to ask, we are hesitant to make any strong assertions about the causal impact of certain team/system organization decisions. However, we have found some noteworthy correlations that are worth exploring further and we believe our general approach could be re purposed to use cases for other data teams.

**Examples of some questions we can ask**:

- "Do organizations with domain-aligned teams have longer lead times than organizations where maintainers cut across team boundaries?"
- "What technology stack is used by organizations where Corporate IT maintains ingestion systems and Domain-aligned Analysts maintain data processing systems?"

**TLDR**:

- The main value of this section is quantifying which joint team/system organization an organization exhibits for a representative group of systems.
- This gives us the tools to make quantitative statements about which organizational properties and business outcomes are associated with certain team/system topologies.
...
# System Distribution by Maintainer Type
...
for (i in 1:nrow(df)) {
  ...
  q1[1,] <- as.numeric(obs[,1:5])
  q1[2,] <- as.numeric(obs[,6:10])
  q1[3,] <- as.numeric(obs[,11:15])
  q1[4,] <- as.numeric(obs[,16:20])
  q1[5,] <- as.numeric(obs[,21:25])
  ...
  cumulative_links = cumulative_links + q1
}

rownames(cumulative_links) = c('sg1', 'sg2', 'sg3', 'sg4', 'sg5')
colnames(cumulative_links) = c('ing', 'proc', 'elem', 'agg', 'fin')
```
...
The diagram below visualizes this matrix

```{r}
#| label: unconditional-maintenance-distribution
#| layout-ncol: 1
#| echo: false
#| warning: false
#| out-width: "100%"
#| fig-cap:
#| - Unconditional Maintenance Distribution
...
## Conditional on various maintainer combinations
...
# rownames(q1) = c('sg1', 'sg2', 'sg3', 'sg4', 'sg5')
# colnames(q1) = c('ing', 'proc', 'elem', 'agg', 'fin')
# 
# rownames(q2) =  c('sg1', 'sg2', 'sg3', 'sg4', 'sg5')
# colnames(q2) = c("Corporate IT", "Domain IT", "Domain Analysts", "E. Analysts", "Consult.")
# 
# CIT_ONLY = q2[,1] == 1 & q2[,2:5] == 0
...
for (i in 1:nrow(df)) {
  ...
  # Select the rows of q2 where **only** Corporate IT is a maintainer,
  # then use this selection to index into q1. The point of this is to
  # get the distribution of system groups across GSBPM steps, given that
  # Corporate IT is the maintainer.
  cit_idx <- q2[,1] == 1 & q2[,2] == 0 & q2[,3] == 0 & q2[,4] == 0 & q2[,5] == 0
  if (any(cit_idx)) {
    cumulative_links[cit_idx] = cumulative_links[cit_idx] + q1[cit_idx]
  }
}
...
E.g., for the first one, it's interpreted as "Looking at system groups where Corporate IT is the **only** maintainer, what is the relationship between system groups and GSBPM steps?"

## Interpreting these matrices

This is a rough measure, but as you move from top left to bottom right, you're implicitly moving along the flow of work (the system groups are listed in left-to-right order from ingestion to finalization if there is more than one system group).

If there's a high concentration in one section of the matrix, this can be interpreted as "more of the system groups maintained **only** by Corporate IT tend to be focused on these parts of the GSBPM steps".
...
```{r}
#| label: cit-only-maintenance-distribution
#| layout-ncol: 1
#| echo: false
#| warning: false
#| out-width: "100%"
#| fig-cap:
#| - Maintenance Distribution Conditional on Corporate IT being the **only** maintainer
...
for (i in 1:nrow(df)) {
  ...
  # get the distribution of system groups across GSBPM steps, given that
  # Corporate IT is the maintainer.
  cit_idx <- q2[,2] == 1 & q2[,1] == 0 & q2[,3] == 0 & q2[,4] == 0 & q2[,5] == 0
  if (any(cit_idx)) {
    cumulative_links[cit_idx] = cumulative_links[cit_idx] + q1[cit_idx]
  }
}
...
Next, we have the maintainer distribution where **Domain-Aligned IT** is the only maintainer.
...
```{r}
#| label: dit-only-maintenance-distribution
#| layout-ncol: 1
#| echo: false
#| warning: false
#| out-width: "100%"
#| fig-cap:
#| - Maintenance Distribution Conditional on Domain-Aligned IT being the **only** maintainer
...
for (i in 1:nrow(df)) {
  ...
  # get the distribution of system groups across GSBPM steps, given that
  # Corporate IT is the maintainer.
  cit_idx <- q2[,3] == 1 & q2[,1] == 0 & q2[,2] == 0 & q2[,4] == 0 & q2[,5] == 0
  if (any(cit_idx)) {
    cumulative_links[cit_idx] = cumulative_links[cit_idx] + q1[cit_idx]
  }
}
...
```
```{r}
#| label: dan-only-maintenance-distribution
#| layout-ncol: 1
#| echo: false
#| warning: false
#| out-width: "100%"
#| fig-cap:
#| - Maintenance Distribution Conditional on Domain-Aligned Analysts being the **only** maintainers
...
for (i in 1:nrow(df)) {
  ...
  # get the distribution of system groups across GSBPM steps, given that
  # Corporate IT is the maintainer.
  cit_idx <- q2[,4] == 1 & q2[,1] == 0 & q2[,2] == 0 & q2[,3] == 0 & q2[,5] == 0
  if (any(cit_idx)) {
    cumulative_links[cit_idx] = cumulative_links[cit_idx] + q1[cit_idx]
  }
}
...
```{r}
#| label: ean-only-maintenance-distribution
#| layout-ncol: 1
#| echo: false
#| warning: false
#| out-width: "100%"
#| fig-cap:
#| - Maintenance Distribution Conditional on Elsewhere Analysts being the **only** maintainers
...
for (i in 1:nrow(df)) {
  ...
  # get the distribution of system groups across GSBPM steps, given that
  # Corporate IT is the maintainer.
  cit_idx <- q2[,5] == 1 & q2[,1] == 0 & q2[,2] == 0 & q2[,3] == 0 & q2[,4] == 0
  if (any(cit_idx)) {
    cumulative_links[cit_idx] = cumulative_links[cit_idx] + q1[cit_idx]
  }
}
...
```{r}
#| label: con-only-maintenance-distribution
#| layout-ncol: 1
#| echo: false
#| warning: false
#| out-width: "100%"
#| fig-cap:
#| - Maintenance Distribution Conditional on Consultants being the **only** maintainers
...
# Systems with Mixed Maintainers

Next, we look at system distributions for systems where more than one kind of team maintains the system.

## Domain-Aligned Maintainers

This first scenario looks at the case when maintenance responsibilities for a system group are split between two **stream-aligned** teams (domain-aligned analysts and domain-aligned IT).

In this situation, we would expect to see a roughly even split across all steps in the flow of work. This is because domain teams own the end-to-end workflow for their product.

Another way to think about this is that domain aligned maintainers **span the required skillset** and are located entirely **within the business domain**.
...
for (i in 1:nrow(df)) {
  ...
  q2[4,] <- as.numeric(obs[,41:45])
  q2[5,] <- as.numeric(obs[,46:50])
  # Select the rows of q2 where domain IT and domain analysts are the maintainers
  # This would correspond to a "stream aligned" team scenario, so we'd expect to
  # see an even distribution of GSBPM steps since domain IT is present for the
  # entire flow of work.
  cit_idx <- q2[,1] == 0 & q2[,2] == 1 & q2[,3] == 1 & q2[,4] == 0 & q2[,5] == 0
  if (any(cit_idx)) {
    cumulative_links[cit_idx] = cumulative_links[cit_idx] + q1[cit_idx]
  }
}
...
**Note**: a possible finding from this might be that stream aligned teams (domain it and domain analysts) tend to maintain systems that span the flow of work, while situations where maintenance is split between corporate (non-stream aligned) IT and domain-aligned analysts tend to be partitioned into central IT handling the "ingestion/processing" part and domain-aligned analysts handling the "processing/EA/agg/finalization" part, even though this cuts across the flow of work.
...
```{r}
#| label: domain-it-analyst-only-maintenance-distribution
#| layout-ncol: 1
#| echo: false
#| warning: false
#| out-width: "100%"
#| fig-cap:
#| - Maintenance Distribution Conditional on Domain-Aligned Analysts **and** Domain-Aligned IT being the maintainers.
...
Small sample, but this does appear to be the case.

## IT-only maintainers

This section looks at the distribution of systems for cases where the maintainers span Corporate IT and Domain-Aligned IT.

This would suggest that regardless of alignment with the business domain, these systems tend to lend themselves to individuals with an IT skill set.
...
for (i in 1:nrow(df)) {
  ...
  # see an even distribution of GSBPM steps since domain IT is present for the
  # entire flow of work.
  cit_idx <- q2[,1] == 1 & q2[,2] == 1 & q2[,3] == 0 & q2[,4] == 0 & q2[,5] == 0
  if (any(cit_idx)) {
    cumulative_links[cit_idx] = cumulative_links[cit_idx] + q1[cit_idx]
  }
}
...
```{r}
#| label: it-only-maintenance-distribution
#| layout-ncol: 1
#| echo: false
#| warning: false
#| out-width: "100%"
#| fig-cap:
#| - Maintenance Distribution Conditional on IT (Corporate or Domain) being the maintainers
...
Sample too small to say anything definitively, but could be worth showing as something interesting for future research.



# Maintainer Breakdown Conditional on Crossing System Boundaries

You can also ask the inverse question: "what does the breakdown of maintainers look like when systems cut across certain GSBPM boundaries?

## Example 1: The "monolith" topology

This is the case where one system group cuts across **all** GSBPM steps
...
for (i in 1:nrow(df)) {
  ...
  # see an even distribution of GSBPM steps since domain IT is present for the
  # entire flow of work.
  cit_idx <- q1[,1] == 1 & q1[,2] == 1 & q1[,3] == 1 & q1[,4] == 1 & q1[,5] == 1
  if (any(cit_idx)) {
    cumulative_links[cit_idx] = cumulative_links[cit_idx] + q2[cit_idx]
  }
}
...
**Note**: I wonder if this further breaks down into system groups following "classic IT monolith" architecture and "one person with an Excel workbook"?

```{r}
#| label: monolith-team-distro
#| layout-ncol: 1
#| echo: false
#| warning: false
#| out-width: "100%"
#| fig-cap:
#| - Team Distribution Conditional on Monolith Architecture
...
## System Boundary Change Between Ingestion and Processing

In this section, we're looking for "cut-overs" that look like the following diagram.

     [,1] [,2] [,3] [,4] [,5]
[i,]    1    0    X    X    X
[j,]    0    1    X    X    X

In other words, when one system group "ends" at ingestion, and the next one "begins" at data processing.

We can look at what the team distribution looks like for system groups that have the property shown above.
...
for (i in 1:nrow(df)) {
  ...
  q2[5,] <- as.numeric(obs[,46:50])
  # 
  cit_idx <- lag(q1[,1]) == q1[,2] & lag(q1[,1]) & q1[,1] != q1[,2]
  cit_idx[is.na(cit_idx)] = TRUE
  if (any(cit_idx)) {
    cumulative_links[cit_idx] = cumulative_links[cit_idx] + q2[cit_idx]
  }
}
...
cumulative_links
```
I would interpret this as "when we see a cut-over between system groups that happens between the ingestion step and the processing step, it's often a cutover between Corporate IT **or** Domain IT (14/22 occurrences in sg1) **to** a system group maintained by domain aligned analysts (10/14 occurrences).
